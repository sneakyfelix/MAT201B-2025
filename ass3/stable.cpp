...
void appendMesh(Mesh &target, const Mesh &source) {
  ...
  Mesh createCatMesh() {
    ...
    // front left leg/paw
    makeBox({0.08f, 0.2f, 0.08f}, {0.2f, -0.225f, 0.15f}, Color(0.45f, 0.27f, 0.07f));
    makeBox({0.08f, 0.05f, 0.08f}, {0.2f, -0.325f, 0.15f}, Color(1)); // white paw
    ...
    makeBox({0.08f, 0.05f, 0.08f}, {-0.2f, -0.225f, -0.15f}, Color(1)); // white paw
...
struct AlloApp : App {
  ...
  void onAnimate(double dt) override {
    ...
    if (cameraMode == 1) {
        // follow cat
        cameraNav.pos(catNav.pos() + catNav.uf() * -5 + Vec3f(0, 2, 0));
        cameraNav.faceToward(catNav.pos(), 1.0);
    }
    ...
    if (time > 7) {
        time -= 7;
        owner = Vec3f(rnd::uniform(-50, 50), 0, rnd::uniform(-50, 50));
    }
    time += dt;

    catNav.faceToward(owner, 0.02);
    catNav.nudgeToward(owner, 0.08);
    catNav.moveF(0.1);
    catNav.step(dt);
  ...
  void onDraw(Graphics &g) override {
    nav().pos(cameraNav.pos());
    nav().quat(cameraNav.quat());
